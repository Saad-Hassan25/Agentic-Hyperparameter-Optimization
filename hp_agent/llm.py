...
deterministic pipeline (seed, evaluate, select, baseline) runs with no `openai`
install and no API key — the LLM is judgment on top of deterministic search,
never a hard dependency.

Responsibilities (both LLM call sites — propose, report — go through here):
...
class LLMClient:
    ...
    def available(self) -> tuple[bool, str]:
        ...
        `model=None` is checked first and unconditionally: it is the
        --no-llm switch for this agent, so the network is never touched
        (not even an `openai` import check) once it is set.
        """
        if self.config.model is None:
            return False, "no model configured (model=None means --no-llm mode)."
        try:
    ...
    def structured(
        ...
        *,
        ...
        the model can correct itself. Hard-fails the *call* (raises), not the run
        — the orchestrator catches this and degrades gracefully (repair fallback,
        or the baseline-sampler path).
        """
